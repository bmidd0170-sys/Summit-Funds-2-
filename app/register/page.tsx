    ...
    <main className="bg-[#F8F9FA] min-h-screen font-sans flex items-center justify-center">
      <div className="auth-container max-w-md w-full px-4 py-12 mx-auto">
        <div className="auth-card bg-white p-10 rounded-2xl shadow-xl">
          <h2 className="auth-title text-3xl font-sora font-bold text-[#2D5F7E] mb-8 text-center">Register</h2>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="form-group">
              <label className="form-label block font-semibold mb-2">Confirm Password:</label>
              <input type="password" className="form-input w-full px-4 py-3 border-2 border-[#E0E4E8] rounded-lg focus:outline-none focus:border-[#2D5F7E] bg-[#F8F9FA]" placeholder="Re-enter your password" required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} />
            </div>
          ...
          <button type="button" className="btn btn-secondary btn-full w-full py-3 bg-[#F8F9FA] text-[#1A1A1A] border-2 border-[#E0E4E8] rounded-lg font-semibold hover:bg-white hover:border-[#2D5F7E] transition mb-2" onClick={handleGoogleRegister} disabled={loading}>
            {loading ? "Please wait..." : "Continue with Google"}
          </button>
          {error && <div className="text-red-600 text-center mt-4">{error}</div>}
          <div className="auth-link text-center mt-6 text-[#5A5A5A]">
            Already have an account?{' '}
            <Link href="/login" className="text-[#2D5F7E] font-semibold hover:underline">Login</Link>
          </div>
        </div>
      </div>
    </main>
